...
const CreateRace = ({ setCreateRace = () => {} }) => {
  ...
  const generateRandomStockRaceName = () => {
    ...
    let encodedUserDetails = localStorage.getItem("fin_userDetails");
    ...
    const randomPhrase =
      stockPhrases[Math.floor(Math.random() * stockPhrases.length)];
    let today = Date.now();
  ...
  const [validation, setValidation] = useState({
    // this will only contain bool value
  ...
  const addStock = () => {
    ...
    theseItems.push({
      stock_id: "",
      prediction_price: 0,
      prediction_rank: theseItems.length + 1,
    });
        ...
        <div className="">
          ...
          <div className="w-full flex gap-[2.5rem] mb-[1.2rem]">
            <div className="flex flex-col flex-1">
              <label className="mb-[10px] dark:text-white" htmlFor="race_name">
                {" "}
                Race Name
              </label>
              <input
                placeholder="Enter the name for Race here"
                value={raceDetails.name}
                onChange={(e) => handleRaceDetails("name", e.target.value)}
          ...
          <div className="mb-4">
            <label className="mb-[10px] dark:text-white font-semibold">
              Quick Duration
            </label>
            <select
              className="ml-3 px-3 py-2 rounded border dark:bg-[#001B51] dark:text-white"
              defaultValue=""
              onChange={(e) => {
                const hours = Number(e.target.value);
                if (hours) handleDurationSelect(hours);
              }}
            >
          ...
          <div className="w-full flex gap-[2.5rem] mb-[1.2rem]">
            <div className="flex flex-col flex-1">
              <label
                className="mb-[10px] dark:text-white"
                htmlFor="race-start-date"
              >
                Start Date
              </label>
              <input
                ...
                onChange={(e) =>
                  handleRaceDetails("start_date", e.target.value)
                }
                onSelect={() => {
            ...
            <div className="flex flex-col flex-1">
              ...
              <input
                ...
                value={raceDetails.start_time}
                onChange={(e) =>
                  handleRaceDetails("start_time", e.target.value)
                }
                id="race-start-time"
                className="px-[1.1rem] rounded-[4px] py-[8px] shadow-inner"
                type="time"
          ...
          <div className="w-full flex gap-[2.5rem] mb-[1.2rem]">
            ...
            <div className="flex flex-col flex-1">
              ...
              <input
                placeholder="Race Starting time"
                value={raceDetails.end_time}
                onChange={(e) => handleRaceDetails("end_time", e.target.value)}
                id="race-end-time"
                className="px-[1.1rem] rounded-[4px] py-[8px] shadow-inner"
                type="time"
                // step="1"
          ...
          <div className="w-full flex flex-col gap-4 mb-[14px]">
            <div className="flex items-center gap-[10px]">
              ...
              <div
                title="Anyone can join open race but only authorized members can join closed race."
                className="relative"
              >
                <img src={info} alt="info icon" />
              </div>
          ...
          <p className="font-semibold mt-12 mb-4 dark:text-white">
            Add Stocks <span className="text-[#838386]">(upto 10)</span>
          </p>
          ...
          <div className="w-full flex flex-col">
            {racePredictions[0] ? (
              racePredictions?.map((curr, index) => {
                return (
                  <StockEntryRow
                    key={index + 1}
              ...
              <div className="w-full text-center text-slate-500 dark:text-white">
                Add some Stocks
              </div>
            )}
          </div>
          <div className="w-full flex justify-center items-center mt-4">
            <button
              title="Click to add more stocks entries"
              onClick={addStock}
              className="pl-[1.5rem] pr-[0.7rem] py-[8px] font-semibold flex gap-2 rounded-[8px] active:scale-95 border border-slate-500 dark:text-white"
            >
              Add Stocks <IoIosAdd size={24} />
            </button>
          </div>
        </div>
        <div className="w-full flex justify-center items-center mt-4">
          <button
            onClick={() => {
              let { start_date, end_date, name, start_time, end_time } =
                raceDetails;
              ...
              if (start_date && end_date) {
                ...
                if (start_date == end_date) {
                  if (end_time < start_time) {
                    alert(
                      "The time is not correct for the race Ending in Same Day"
                    );
                    return;
                  }
                }
              ...
              if (racePredictions.length < 2) {
                ...
                racePredictions.forEach((element) => {
                  ...
                  if (
                    element.prediction_price < 0 ||
                    element.prediction_rank <= 0
                  ) {
                    alert(
                      "Predicted price can't be negative. Please verify your values."
                    );
                    return;
                  }
                });
              ...
              let actualRacePredictions = racePredictions.map((curr) => {
                if (curr.value_type_percent) {
                  return {
                    prediction_price:
                      curr.prediction_price * (curr.current_price / 100) +
                      curr.current_price,
                    prediction_rank: curr.prediction_rank,
              ...
              createRaceAndJoinUser(
                raceDetails,
                actualRacePredictions,
                (res) => {
                  setRaceDetails(
                    {
                      isSimulation: false,
