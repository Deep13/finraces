                ...
                >
                    ...
                    {inviteSent ? (
                        <div className="flex justify-center items-center flex-col my-4">
                            ...
                            <p className="text-green-600 font-semibold mt-2 text-xl">Invite sent!</p>
                            <p onClick={() => { setInviteSent(false) }} className="text-blue-500 hover:underline cursor-pointer text-sm">Invite More Friends</p>
                        </div>
                    ) : (
                            ...
                            <div className="w-[60%] max-h-40 overflow-y-auto mt-2">
                                {
                                    friendList?.length > 0 ? (
                                                        ...
                                                        onClick={() => {
                                                            setSelectedFriends(prev =>
                                                                isSelected
                                                                    ...
                                                                    : [...prev, { id: friend.id, name: `${friend.firstName} ${friend.lastName}` }]
                                                            );
                                                            setSearchQuery("");
                                                            setFriendList([]);
                                                        }}
                                        ...
                                        searchQuery.length > 0 && (
                                            <p className="text-sm text-gray-400 text-center">No friends found.</p>
                                        )
                                    )
                                }
                                ...
                                {selectedFriends.length > 0 && (
                                    <div className="mt-3 w-full">
                                        <p className="font-semibold mb-1 text-center">Selected Friends:</p>
                                        <div className="flex flex-wrap gap-2 justify-center mb-2">
                                            {selectedFriends.map(({ id, name }) => (
                                                ...
                                                >
                                                    {name}
                                                    <button
                                                        onClick={() => setSelectedFriends(prev => prev.filter(f => f.id !== id))}
                                        ...
                                        <div className="flex justify-center">
                                            <button
                                                onClick={() => {
                                                    sendRaceInvite(race_id,selectedFriends,()=>{
                                                        setSelectedFriends([]);
                                                        setInviteSent(true);
                                                    },(error)=>{
                                                        console.log(error)
                    ...
                    <div className='w-full flex-1 text-center flex flex-col items-center gap-[5px] overflow-y-auto joining-users' style={{ maxHeight: '180px' }}>
                        {joinedUsersList?.map((curr, index) => (
                            <p key={index}>{checkSelf(curr?.user?.id, curr?.user?.firstName)} joined successfully</p>
                        ))}
                        {liveUsers?.map((curr, index) => (
                            <p key={`live-${index}`}>{checkSelf(curr?.id, curr?.firstName)} joined successfully</p>
                        ))}
                    </div>
