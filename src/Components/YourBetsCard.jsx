...
const YourBetsCard = ({
  ...
  // }, [])
  return (
    <div className="p-4 dark:bg-[#002763] border-2 dark:border-[#00387E] rounded-[8px] flex justify-between gap-5 ">
      <div className="flex flex-1 gap-2">
        <div className="rounded-full bg-white">
          {!imageUrl && (
            <div className="w-12 h-12 bg-gradient-to-l rounded-full from-[#005BFF] to-[#5B89FF] dark:text-white font-bold grid place-items-center">
              {stockName.substring(0, 2)}
            </div>
          )}
          {imageUrl && (
            <img
              className="w-12 h-12 bg-white object-cover rounded-full"
              src={imageUrl}
              alt=""
            />
          )}
        ...
        <div className="flex flex-col justify-center items-start line-clamp-3 w-[50%]">
          <p className="text-[0.7rem] font-semibold dark:text-white line-clamp-2">
            {stockName}
          </p>
        </div>
      </div>
