        ...
        <div className="flex flex-col">
          <div className="w-full flex justify-between items-center">
            <p
              ...
              className="mb-[0.4rem] font-bold text-[2rem] dark:text-white max-w-[75%] hover:underline cursor-pointer font-poppins"
            >
              {raceName}
            </p>
            <div className="flex gap-[6px] items-center self-start flex-wrap">
              ...
              <p className="font-semibold text-[16px] text-[#b5b5b5]">
                {totalStocksCount > 3 ? `+${totalStocksCount - 3}` : ""}
              </p>
            </div>
          </div>
          <div className="w-full flex justify-between items-center mb-[5px]">
            <div className="flex justify-between items-baseline">
              <p className="font-bold text-[20px] mr-[1rem] dark:text-white">
                Race will start at{" "}
                <span className="font-poppins">
                  {new Date(startDate).toLocaleTimeString(navigator.language, {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </span>{" "}
                <span className="font-poppins">on {StartDate}</span>
              </p>
            </div>
          </div>
          <div className="flex flex-col gap-[5px] mb-[14px]">
            <p className="text-[12px] font-bold dark:text-white">
              <span className="font-poppins">{participants}</span>
              {participants > 1 ? " Participants" : " Participant"}
            </p>
            <p className="text-[12px] dark:text-white font-poppins">
          ...
          <button
            onClick={(e) => {
              ...
              if (userDetails || guestDetails) {
                ...
                x = JSON.parse(atob(x));
                let aData = participantsData?.filter(
                  (curr) => curr.id === x.userId
                );
                if (aData.length == 0) {
