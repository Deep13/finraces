        ...
        <div className='max-w-[1400px] relative mb-[3.29rem] grid grid-cols-1 md:grid-cols-2 gap-[2rem]'>
            <div className='mb-[2rem] flex-1'>
                ...
                <div className='w-[60%] flex flex-col gap-[8px]'>
                    ...
                    <button onClick={()=>{setReport(true)}} className='rounded-[20.62px] bg-[#e5f4ff] text-[1.21rem] font-extrabold text-start p-[1rem] relative dark:bg-gradient-to-r from-[#018ab7] to-[#125399]  dark:text-white flex justify-between items-center'>Ask for more help <span className=""><AiOutlineArrowRight size={27} /></span></button>
                </div>
            </div>
            <div className='h-full flex flex-col justify-start gap-[8px]'>
                {accordionData.map((item, index) => (
                    <div key={index} onClick={() => toggleAccordion(index)} className="w-full dark:bg-gradient-to-r from-[#018ab7] from-0% via-10% to-100% to-[#125399]  dark:text-white py-[1.41rem] flex flex-col items-center bg-[#e5f4ff] rounded-[20.6px] px-[2.35rem] relative cursor-pointer">
