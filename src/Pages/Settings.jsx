        ...
        <div className="w-full dark:bg-[#000A2D] dark:border dark:border-[#00387E] rounded-[24px] h-full pb-6 ">
          <div className="dark:text-white w-full h-full">
            <div className="w-full flex flex-col gap-4 justify-start mb-4">
              <p className="font-semibold text-[1.4rem] py-4 px-6 flex items-center gap-3">
              ...
              <div className="w-full h-full flex gap-4">
                ...
                <div className=" w-[14rem] flex flex-col gap-4 h-full">
                  {Object.keys(tabs).map((curr, index) => {
                    return (
                      <button
                        onClick={() => setActiveTab(tabs[curr].title)}
                        key={curr}
                        className={`w-full flex gap-2 items-center roudned-xl dark:text-white bg-[#e5f4ff] text-[0.9rem] ${
                          activeTab === tabs[curr].title
                            ? "dark:bg-[#002763]  border border-[#00387E] text-black"
                            : "dark:bg-[#001B51]"
                        } py-4 px-[1.2rem] rounded-[15px] text-start`}
                      >
                        <img
                          src={tabs[curr].icon}
                          alt="curr"
                ...
                {/* active tab  */}
                <div className="flex-1 bg-[#e5f4ff] rounded-xl dark:bg-[#001B51] border dark:border-[#00387E] dark:text-white py-4 px-6 h-full">
                  {activeTab === tabs["User Preferences"].title && (
                    <UserPreferences />
                  )}
                  {/* {activeTab === tabs["Security"].title && <Security />} */}
                  {activeTab === tabs["Help and Support"].title && (
                    <HelpAndSupport />
                  )}
                  {activeTab === tabs["Privacy Settings"].title && (
                    <PrivacySettings />
                  )}
                  {activeTab === tabs["Account Management"].title && (
                    <AccountManagement />
                  )}
                </div>
              </div>
